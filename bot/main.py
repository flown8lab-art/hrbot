...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await message.reply_text(
        "<b>Привет, я помогу тебе найти работу быстрее</b> 🚀\n\n"
        "Давай начнем\n\n"
        "<b>Шаг 1 из 3:</b>\n"
        "Для начала загрузи свое резюме в формате (PDF, DOCX или текст)",
        parse_mode="HTML")
...
async def receive_resume(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.document:
        ...
        skip_kb = InlineKeyboardMarkup([[
            InlineKeyboardButton("Пропустить ➡️",
                                 callback_data="skip_preferences")
        ]])
        await update.message.reply_text(
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await message.reply_text(
        ...
        "Что ты получаешь:\n"
        "🔎 <b>Не тратишь время на поиск</b> — все актуальные вакансии из 130+ источников уже собраны для тебя.\n"
...
def main():
    ...
    async def post_init(app):
        await app.bot.set_my_commands([("start", "Начать поиск работы"),
